...
3. **Compliance system needs two updates:**
   - Add `[3.1-cr]` check for credit instruments at 7% (currently only checks `[3.1-eq]` at 10%)
   - Implement Rule 0.1 auto-satisfaction of 6.1 criterion 1 (or simplify the scoring system)
...
|-----------|----------|---------|
| **Income Bridge** | SAFE | Covers 108 months of expenses (84 months excess over 24-month floor). A 176% decline in stabiliser holdings would be needed to break it. |
...
| **AUD Liability Matching** | SAFE | AUD growth at 57.3%. A 34.3% rally in non-AUD growth would be needed to weaken liability matching below 50%. |
...
| **AUD Liability Matching** | FRAGILE | AUD growth at 50.4%. A **1.7% rally** in non-AUD growth (VGS, PMGOLD, FLBL) weakens liability matching below 50%. Monthly monitoring required. |
...
### Supporting: Rule-Level Buffers (Post-Trade)

| Rule | Description | Current | Limit | Buffer | Breach Move |
|------|-------------|--------:|------:|-------:|-------------|
| 3.1-eq | VAS.AX equity cap | 9.9% | 10% | **0.1pp** | VAS.AX rallies 1% (rest flat) |
| 7.2 | Duration: bucket 1y | 39.9% | 40% | **0.1pp** | Other stabiliser assets decline |
| 3.1-eq | VGS.AX equity cap | 9.8% | 10% | **0.2pp** | VGS.AX rallies 2% (rest flat) |
| 3.1-eq | PMGOLD.AX equity cap | 9.8% | 10% | **0.2pp** | PMGOLD.AX rallies 2% (rest flat) |
| 3.1-eq | BHP.AX equity cap | 9.5% | 10% | 0.5pp | BHP.AX rallies 6% (rest flat) |
| 3.2 | Issuer: AU Government | 18.2% | 20% | 1.8pp | Group outperforms rest |
| 3.1-eq | FLBL equity cap | 7.0% | 10% | 3.0pp | FLBL rallies 48% (rest flat) |

---

## Appendix B: Stress Scenario Impact on Objectives

*For each scenario: can you still feed your family? How much compounding was destroyed? Did optionality perform?*
...
| **COVID-19 2020** | Pre | AUD 320,584 | AUD 257,022 | 6.1 yrs | 101 months | No |
| | **Post** | **AUD 446,863** | **AUD 355,243** | **6.0 yrs** | **47 months** | **No** |
| | Delta | +126,279 | +98,221 | -0.1 | -54 months | |
| **GFC 2008** | Pre | AUD 476,996 | AUD 396,421 | 10.8 yrs | 100 months | No |
| | **Post** | **AUD 661,739** | **AUD 544,627** | **10.5 yrs** | **47 months** | **No** |
| | Delta | +184,743 | +148,206 | -0.3 | -53 months | |
| **2022 Rate Shock** | Pre | AUD 38,000 | gained 7,343 | 0 yrs | 103 months | No |
| | **Post** | **AUD 152,216** | **AUD 70,012** | **1.0 yrs** | **45 months** | **No** |
| | Delta | +114,216 | +77,355 | +1.0 | -58 months | |
...
- **More dollars at risk in stress** — because there are more dollars deployed in growth assets. Post-trade GFC losses rise from AUD 477k to AUD 662k (+39%). This is the mathematical consequence of putting capital to work.
...
- **Recovery time is essentially unchanged** — 6.8 years in both portfolios for a flat 35% haircut. For GFC, it actually improves slightly (10.8→10.5 years) because the post-trade portfolio is more diversified.
- **No forced liquidation in any scenario** — the critical survivability test passes for both portfolios. Stabiliser covers 45–50 months even post-GFC.
- **Optionality now exists** — post-trade, AUD 212,285 of optionality absorbs stress losses. In the flat 35% scenario, optionality lost AUD 74k (35% of 212k), but in a real gold-driven crisis PMGOLD would be expected to rally, not fall with equities. The proxy drawdowns are worst-case.

### Post-Trade Objective Assessment by Scenario

**1. SURVIVABILITY: Pass in all scenarios (both portfolios).** Post-trade stabiliser AUD 446,515 still covers 45–50 months under the worst stress. Forced liquidation threshold is never close.
...
**3. COMPOUNDING: More capital at risk, but by design.** Post-trade GFC destroys AUD 545k vs pre-trade AUD 396k. But the pre-trade portfolio had only AUD 801k compounding — the post-trade has AUD 1,123k. The percentage loss is actually slightly lower (48% vs 49%) because of broader diversification. More dollars at risk is the price of having more dollars compounding.

**4. OPTIONALITY: Now meaningful, but untested.** At AUD 212,285, optionality is 12% of the portfolio. In the proxy-based scenarios (which assume gold/silver lose alongside equities), optionality lost 30–45%. In reality, gold rallied +26% during COVID 2020 and +5% during the GFC. If PMGOLD performed to its historical pattern, it would offset AUD 45–100k of compounder losses — changing the character of the portfolio from "survive and absorb" to "survive and partially offset."

**5. 2022 RATE SHOCK: The post-trade vulnerability.** The 2022 scenario is the most damaging for the post-trade portfolio: AUD 152k lost vs pre-trade AUD 38k. This is because bonds, credit, AND equities all fell simultaneously, and the post-trade portfolio has more assets in all three. Gold also fell ~4% in 2022 rate shock (proxy: -20%). This scenario specifically targets the post-trade design.
...
| **Does optionality provide crisis alpha?** | Unknown — insufficient data. | Only GHY.AX (AUD 7,285) is classified as optionality. Too small and too few instruments for meaningful measurement. |
...
**Post-Trade Compliance (projected):** 17 pass, 1 monitor, 0 breach
**Analytics added:** 2026-02-18 — Objective-Level Sensitivity (pre/post), Stress Impact (pre/post comparison), Diversification Quality
